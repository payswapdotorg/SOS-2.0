/**
 * THE REAL HTTP SEAM (Work Order P17-A, deployment lane) — the single
 * place real network enters the deployment-provider surface. Library
 * code NEVER touches global fetch, ambient timers, the ambient environment or any
 * provider SDK: the Vercel REST client receives a FetchPort and
 * performs typed requests through it. The process boundary binds the
 * port to the platform's global fetch (bindGlobalFetch — the documented
 * impure boundary, the P17-B/P17-C seam discipline).
 */
...
/** Decode a response body as UTF-8 text (pure). */
...
/** UTF-8 encode a JSON request body (pure). */
