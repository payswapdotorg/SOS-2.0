...
export default async function Page({ searchParams }: { searchParams: Promise<{ key?: string }> }) {
  ...
  const view = key.length > 0 && ledger !== null ? ledger.get(key) : undefined;
      ...
      {view === undefined ? (
        <div className="rounded-xl border border-epistemic/40 bg-epistemic-soft p-5" role="note">
          ...
          <p className="mt-2 text-sm text-ink-soft">
            The receipt ledger is IN-PROCESS by design (the durable store adapters are UNAVAILABLE from this environment — no durable confirmation is claimed). A receipt key is only resolvable in the process that executed the action; this instance did not execute it (cold start or a followed stale link). The action outcome is not fabricated from the key alone.
          </p>
          <p className="mt-3">
